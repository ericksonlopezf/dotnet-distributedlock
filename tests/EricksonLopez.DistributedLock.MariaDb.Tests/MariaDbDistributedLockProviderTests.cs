...
public sealed class MariaDbDistributedLockProviderTests
{
    private readonly NullLogger<MariaDbDistributedLockProvider> _logger = NullLogger<MariaDbDistributedLockProvider>.Instance;

    [Fact]
    public void Constructor_WithConnection_NullConnection_ThrowsArgumentNullException()
    {
        var act = () => new MariaDbDistributedLockProvider((IDbConnection)null!, _logger);
    ...
    public void Constructor_WithConnection_NullLogger_ThrowsArgumentNullException()
    {
        ...
        var act = () => new MariaDbDistributedLockProvider(conn, null!);
    ...
    {
        var act = () => new MariaDbDistributedLockProvider((Func<DbConnection>)null!, _logger);
    ...
    {
        var act = () => new MariaDbDistributedLockProvider(() => new FakeDbConnection(), null!);
        act.Should().Throw<ArgumentNullException>().WithParameterName("logger");
    }
    ...
    {
        var options = Options.Create(new MariaDbLockOptions { CommandTimeoutSeconds = 40 });
        var provider = new MariaDbDistributedLockProvider(() => new FakeDbConnection(), _logger, options);
    ...
    public async Task TryAcquireHandleAsync_SessionMode_Success_OpensClosedConnectionAndReturnsHandle()
    {
        ...
        var provider = new MariaDbDistributedLockProvider(
            () =>
            {
            ...
            _logger,
            new MariaDbLockOptions { CommandTimeoutSeconds = 20 });
    ...
    public async Task TryAcquireHandleAsync_SessionMode_UnexpectedException_RethrowsAndDisposes()
    {
        ...
        var provider = new MariaDbDistributedLockProvider(
            ...
            {
                createdConn = new FakeDbConnection();
                createdConn.ExecuteScalarAsyncHandler = (_, _) => throw new InvalidOperationException("Fatal MariaDB connection drop");
                return createdConn;
            },
            _logger);

        var act = () => provider.TryAcquireHandleAsync("res-fatal");
        await act.Should().ThrowAsync<InvalidOperationException>().WithMessage("*Fatal MariaDB connection drop*");
    ...
    {
        var provider = new MariaDbDistributedLockProvider(() => new FakeDbConnection(), _logger);
    ...
    public async Task TryAcquireHandleAsync_AmbientMode_DbConnection_Success()
    {
        ...
        var provider = new MariaDbDistributedLockProvider(conn, _logger);
    ...
    public async Task TryAcquireHandleAsync_AmbientMode_NonDbConnection_Success()
    {
        ...
        var provider = new MariaDbDistributedLockProvider(syncConn, _logger);
    ...
    public async Task TryAcquireHandleAsync_WithExplicitOptions_PropagatesCommandTimeout()
    {
        ...
        var provider = new MariaDbDistributedLockProvider(conn, _logger, new MariaDbLockOptions { CommandTimeoutSeconds = 77 });
