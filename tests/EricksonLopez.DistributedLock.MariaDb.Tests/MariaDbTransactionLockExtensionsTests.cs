...
public sealed class MariaDbTransactionLockExtensionsTests
{
    [Fact]
    ...
    [Fact]
    public async Task MariaDbTransactionLockHandle_DisposeAsync_IsIdempotent()
    {
        var conn = new FakeDbConnection();
        var releaseCount = 0;
    ...
    [Fact]
    public async Task MariaDbTransactionLockHandle_DisposeAsync_WhenConnectionClosed_SkipsRelease()
    {
        var conn = new FakeDbConnection();
        conn.ExecuteScalarAsyncHandler = (_, _) => Task.FromResult<object?>(1);
    ...
    [Fact]
    public async Task MariaDbTransactionLockHandle_DisposeAsync_WhenExceptionThrown_SwallowsGracefully()
    {
        var conn = new FakeDbConnection();
        conn.ExecuteScalarAsyncHandler = (_, _) => Task.FromResult<object?>(1);
    ...
    [Fact]
    public async Task MariaDbTransactionLockHandle_DisposeAsync_WithNonDbConnection_ExecutesReleaseLock()
    {
        var conn = new FakeSyncConnection();
        var executed = false;
