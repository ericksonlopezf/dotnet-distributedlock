...
public sealed class SqlServerDistributedLockHandleTests
{
    [Fact]
    public void Constructor_NullConnection_ThrowsArgumentNullException()
    {
        var act = () => new SqlServerDistributedLockHandle(
            null!,
            "res-key",
            "res-id",
            "Session",
    ...
    [Fact]
    public void Constructor_NullLockOwner_ThrowsArgumentNullException()
    {
        var conn = new FakeDbConnection();
        var act = () => new SqlServerDistributedLockHandle(
            conn,
        ...
        act.Should().Throw<ArgumentNullException>().WithParameterName("lockOwner");
    }
    ...
    {
        var conn = new FakeDbConnection();
        var expectedHash = BitConverter.ToInt64(SHA256.HashData(Encoding.UTF8.GetBytes("order-456")), 0);

        await using var handle = new SqlServerDistributedLockHandle(
            conn,
            "order-456",
            "order-456",
            "Session",
        ...
        handle.ResourceId.Should().Be("order-456");
    ...
    public async Task DisposeAsync_ReleasesLockAndDisposesConnection()
    {
        ...
        var handle = new SqlServerDistributedLockHandle(
            conn,
            "my-lock-resource",
            "my-resource",
            "Session",
        ...
        releaseCommand!.CommandText.Should().Contain("sys.sp_releaseapplock");
        releaseCommand.CommandTimeout.Should().Be(25);
        releaseCommand.Parameters["@Resource"].Value.Should().Be("my-lock-resource");
        releaseCommand.Parameters["@LockOwner"].Value.Should().Be("Session");
    ...
    {
        var conn = new FakeDbConnection();
        conn.ExecuteNonQueryAsyncHandler = (_, _) => throw new InvalidOperationException("Connection broken");
    ...
    [Fact]
    public async Task Keepalive_WhenAlreadyDisposed_DoesNotExecuteQuery()
    {
        var conn = new FakeDbConnection();
        var queryExecuted = false;
        ...
        var handle = new SqlServerDistributedLockHandle(
            ...
            "Session",
            NullLogger.Instance,
            keepaliveCadence: TimeSpan.FromMilliseconds(50));
    ...
    public async Task ExecuteKeepalive_WhenDisposed_ReturnsEarlyWithoutQuery()
    {
        ...
        var method = typeof(SqlServerDistributedLockHandle).GetMethod("ExecuteKeepalive", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
    ...
    public void ExecuteKeepalive_WhenCtsDisposed_HandlesObjectDisposedExceptionGracefully()
    {
        ...
        var ctsField = typeof(SqlServerDistributedLockHandle).GetField("_handleLostCts", System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
