...
/// <summary>
/// Represents an active Oracle Database lock handle managed via DBMS_LOCK that releases the lock upon disposal.
/// </summary>
public sealed class OracleDistributedLockHandle : IDistributedLockHandle
{
    private readonly DbConnection _connection;
    private readonly string _lockHandle;
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="OracleDistributedLockHandle"/> class with the specified connection and lock parameters.
    /// </summary>
    /// <param name="connection">The underlying database connection holding the DBMS_LOCK allocation.</param>
    /// <param name="lockHandle">The unique handle string allocated by DBMS_LOCK.ALLOCATE_UNIQUE.</param>
    ...
    /// <exception cref="ArgumentNullException"><paramref name="connection"/>, <paramref name="lockHandle"/>, <paramref name="resourceId"/>, or <paramref name="logger"/> is <see langword="null"/></exception>
    public OracleDistributedLockHandle(
        ...
        int commandTimeoutSeconds = 30)
    {
        ...
        _lockHandle = lockHandle ?? throw new ArgumentNullException(nameof(lockHandle));
    ...
    private async Task RunKeepaliveLoopAsync(TimeSpan cadence, CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (await timer.WaitForNextTickAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                {
                    _logger.LogWarning("Oracle lock connection is no longer open for resource '{ResourceId}'. Triggering HandleLostToken.", _resourceId);
                ...
                {
                    _logger.LogWarning(ex, "Oracle lock keepalive failed for resource '{ResourceId}'. Triggering HandleLostToken.", _resourceId);
                    SafeCancelHandleLost();
                    DistributedLockMetrics.RecordLockLost(_resourceId, _lockId);
                    break;
                }
                ...
                if (_connection.State == ConnectionState.Open)
                {
                    ...
                    command.CommandText = """
                        DECLARE
                            v_res INTEGER;
                        BEGIN
                            v_res := DBMS_LOCK.RELEASE(:LockHandle);
                        END;
                        """;
                    command.CommandTimeout = _commandTimeoutSeconds;

                    var p = command.CreateParameter();
                    p.ParameterName = "LockHandle";
                    p.Value = _lockHandle;
                    command.Parameters.Add(p);

                    await command.ExecuteNonQueryAsync().ConfigureAwait(false);
                    _logger.LogInformation("Released Oracle lock for resource '{ResourceId}'.", _resourceId);
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Error releasing Oracle lock for resource '{ResourceId}'. The connection will be closed.", _resourceId);
            }
            finally
            {
