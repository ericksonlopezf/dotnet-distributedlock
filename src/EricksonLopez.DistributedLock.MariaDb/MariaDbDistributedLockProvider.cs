...
/// <summary>
/// Provides distributed locks backed by MariaDB user-level locking functions.
/// </summary>
public sealed class MariaDbDistributedLockProvider : IDistributedLockProvider
{
    private readonly IDbConnection? _connection;
    private readonly Func<DbConnection>? _connectionFactory;
    private readonly ILogger<MariaDbDistributedLockProvider> _logger;
    private readonly MariaDbLockOptions _options;

    /// <summary>
    /// Initializes a new instance of the <see cref="MariaDbDistributedLockProvider"/> class using an existing database connection.
    /// </summary>
    ...
    public MariaDbDistributedLockProvider(
        IDbConnection connection,
        ILogger<MariaDbDistributedLockProvider> logger,
        MariaDbLockOptions? options = null)
    {
        _connection = connection ?? throw new ArgumentNullException(nameof(connection));
        _logger = logger ?? throw new ArgumentNullException(nameof(logger));
        _options = options ?? new MariaDbLockOptions();
    }

    /// <summary>
    /// Initializes a new instance of the <see cref="MariaDbDistributedLockProvider"/> class using a connection factory for dedicated sessions.
    /// </summary>
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MariaDbDistributedLockProvider"/> class configured via options snapshot.
    /// </summary>
    /// <param name="connectionFactory">The delegate producing dedicated database connections per lock handle.</param>
    /// <param name="logger">The logger used for recording diagnostic and lifecycle events.</param>
    ...
    public MariaDbDistributedLockProvider(
        Func<DbConnection> connectionFactory,
        ILogger<MariaDbDistributedLockProvider> logger,
        IOptions<MariaDbLockOptions> options)
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_connectionFactory is not null)
        {
            ...
            try
            {
                ...
                if (acquired)
                {
                    _logger.LogInformation("Successfully acquired MariaDB lock for resource '{ResourceId}'.", resourceId);
                    DistributedLockMetrics.RecordAcquisition(resourceId, "session", "acquired");
                    return Result<IDistributedLockHandle>.Success(
                        new MariaDbDistributedLockHandle(connection, lockResource, resourceId, _logger, _options.KeepaliveCadence, _options.CommandTimeoutSeconds));
                }

                _logger.LogWarning("Failed to acquire MariaDB lock for resource '{ResourceId}'. Currently held.", resourceId);
            ...
            {
                _logger.LogError(ex, "Unexpected error acquiring MariaDB lock for resource '{ResourceId}'.", resourceId);
        ...
        try
        {
            ...
            if (acquired)
            {
                _logger.LogInformation("Successfully acquired MariaDB lock for resource '{ResourceId}'.", resourceId);
                DistributedLockMetrics.RecordAcquisition(resourceId, "session", "acquired");
                return Result<IDistributedLockHandle>.Success(new MariaDbAmbientLockHandle(_connection!, resourceId, lockResource, _logger, _options.CommandTimeoutSeconds));
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_connectionFactory is not null)
        {
            ...
            try
            {
                ...
                if (acquired)
                {
                    _logger.LogInformation("Successfully acquired MariaDB lock for resource '{ResourceId}' with timeout {Timeout}.", resourceId, timeout);
                    DistributedLockMetrics.RecordAcquisition(resourceId, "session", "acquired");
                    return Result<IDistributedLockHandle>.Success(
                        new MariaDbDistributedLockHandle(connection, lockResource, resourceId, _logger, _options.KeepaliveCadence, _options.CommandTimeoutSeconds));
                }

                _logger.LogWarning("Failed to acquire MariaDB lock for resource '{ResourceId}' within timeout {Timeout}.", resourceId, timeout);
            ...
            {
                _logger.LogError(ex, "Unexpected error acquiring MariaDB lock with timeout for resource '{ResourceId}'.", resourceId);
                DistributedLockMetrics.RecordAcquisition(resourceId, "session", "error");
                await connection.DisposeAsync().ConfigureAwait(false);
                throw;
            }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        // In MariaDB GET_LOCK, passing a very large timeout simulates blocking indefinitely
        ...
        if (_connectionFactory is not null)
        {
            ...
            try
            {
                ...
                if (acquired)
                {
                    _logger.LogInformation("Successfully acquired blocking MariaDB lock for resource '{ResourceId}'.", resourceId);
                    DistributedLockMetrics.RecordAcquisition(resourceId, "session", "acquired");
                    return Result<IDistributedLockHandle>.Success(
                        new MariaDbDistributedLockHandle(connection, lockResource, resourceId, _logger, _options.KeepaliveCadence, _options.CommandTimeoutSeconds));
                }

                _logger.LogWarning("Failed to acquire blocking MariaDB lock for resource '{ResourceId}'.", resourceId);
                DistributedLockMetrics.RecordAcquisition(resourceId, "session", "already_held");
                await connection.DisposeAsync().ConfigureAwait(false);
                return DistributedLockErrors.LockAlreadyHeld;
            }
            ...
            {
                _logger.LogError(ex, "Unexpected error acquiring blocking MariaDB lock for resource '{ResourceId}'.", resourceId);
                DistributedLockMetrics.RecordAcquisition(resourceId, "session", "error");
                await connection.DisposeAsync().ConfigureAwait(false);
                throw;
            }
    ...
    private sealed class MariaDbAmbientLockHandle : IDistributedLockHandle
    {
        private readonly IDbConnection _connection;
        ...
        public MariaDbAmbientLockHandle(
            IDbConnection connection,
            string resourceId,
            string lockResource,
            ILogger logger,
        ...
        public async ValueTask DisposeAsync()
        {
            ...
            try
            {
                if (_connection.State == ConnectionState.Open)
                {
                    ...
                    _logger.LogInformation("Released MariaDB ambient lock for resource '{ResourceId}'.", ResourceId);
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Error releasing MariaDB ambient lock for resource '{ResourceId}'.", ResourceId);
            }
        }
    }
}
