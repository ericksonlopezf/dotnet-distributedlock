...
/// <summary>
/// Represents an active MySQL user-level lock handle that releases the lock upon disposal.
/// </summary>
public sealed class MySqlDistributedLockHandle : IDistributedLockHandle
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="MySqlDistributedLockHandle"/> class with the specified connection and lock parameters.
    /// </summary>
    ...
    public MySqlDistributedLockHandle(
    ...
    private async Task RunKeepaliveLoopAsync(TimeSpan cadence, CancellationToken cancellationToken)
    {
        ...
        try
        {
            while (await timer.WaitForNextTickAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                {
                    _logger.LogWarning("MySQL lock connection is no longer open for resource '{ResourceId}'. Triggering HandleLostToken.", _resourceId);
                ...
                {
                    _logger.LogWarning(ex, "MySQL lock keepalive failed for resource '{ResourceId}'. Triggering HandleLostToken.", _resourceId);
                    SafeCancelHandleLost();
                    DistributedLockMetrics.RecordLockLost(_resourceId, _lockId);
                    break;
                }
        ...
        finally
        {
            ...
            try
            {
                if (_connection.State == ConnectionState.Open)
                {
                    ...
                    _logger.LogInformation("Released MySQL lock for resource '{ResourceId}'.", _resourceId);
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Error releasing MySQL lock for resource '{ResourceId}'. The connection will be closed.", _resourceId);
            }
            finally
            {
