...
/// <summary>
/// Provides extension methods for acquiring MySQL locks within active database transactions.
/// </summary>
public static class MySqlTransactionLockExtensions
{
    /// <summary>
    /// Attempts to acquire an exclusive transaction-scoped lock on the specified MySQL transaction immediately.
    /// </summary>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (connection is DbConnection dbConn)
        {
            ...
            if (res is not null && !Convert.IsDBNull(res) && Convert.ToInt32(res, CultureInfo.InvariantCulture) == 1)
            {
                ...
                return Result<IDistributedLockHandle>.Success(new MySqlTransactionLockHandle(resourceId, connection, lockResource, logger));
            }
    ...
    /// <summary>
    /// Acquires an exclusive transaction-scoped lock on the specified MySQL transaction, blocking until granted.
    /// </summary>
    /// <param name="transaction">The active database transaction to bind the lock to.</param>
    /// <param name="resourceId">The unique identifier of the resource to lock.</param>
    /// <param name="logger">The logger used for recording diagnostic and lifecycle events.</param>
    ...
    private sealed class MySqlTransactionLockHandle : IDistributedLockHandle
    {
        ...
        public MySqlTransactionLockHandle(string resourceId, IDbConnection connection, string lockResource, ILogger logger)
        {
        ...
        public async ValueTask DisposeAsync()
        {
            ...
            {
                _logger.LogWarning(ex, "Error releasing MySQL lock for resource '{ResourceId}'.", ResourceId);
            }
        }
    }
}
